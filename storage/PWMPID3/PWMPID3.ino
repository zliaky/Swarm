...
#define beta 127 //机器人坐标系0°正方向与全局坐标系X轴正方向夹角
...
void recvFromCenter() {
  if (Serial3.available() >= sizeof(CenterFrame)) {
    ...
    if (start == '~' && start1 == '~') {
      ...
      if (*cFrame.frameEnd == '!'&& (*cFrame.checkSum == (*cFrame.x + *cFrame.y))&& *cFrame.id == 3) {
        // 
...
void getData(){
  ...
  if (cur.X > 5 && cur.Y > 5 && cur.X < 1400 && cur.Y < 600) {
     ...
     valid=cur;
     serialPrint(3, valid.X, valid.Y, valid.Angle);
  }
   Serial3.flush();
 }

void loop() { 
  //  motor.motorMove_2(200, 200,200, 1,1,1);
  displayMode();
  recvFromCenter();
  delay(10);
}
...
void controlDirection(int curAngle) {
  ...
  if(rotatePWM>3){
   ...
   if(rotatePWM<=3){
      countRotate=0;
      for(int i=0;i<120;i++){
        motor.motorMove_2(0,255,130,0,1,0);
